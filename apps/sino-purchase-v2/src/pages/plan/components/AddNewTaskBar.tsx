...
export function AddNewTaskBar({ onAdd, onOpenAdd, onBatch }: Props) {
  ...
  function handleKeyDown(e: React.KeyboardEvent) {
    if (e.key === "Enter") {
      e.preventDefault()
      if (text.trim()) {
        onAdd({ ...defaultInitial, name: text.trim() })
        setText("")
      }
    }
  }
  ...
  return (
    <div className="add-bar" style={{ display: "flex", gap: 4, alignItems: "center", padding: "4px 0", flexShrink: 0 }}>
      <InputGroup
        inputRef={inputRef}
        placeholder="品名 回车快速添加"
        value={text}
        onChange={e => setText(e.target.value)}
        onKeyDown={handleKeyDown}
        style={{ flex: 1 }}
      />
      <Button small icon={<Icon icon={IconNames.NEW_OBJECT} />} onClick={onOpenAdd}>
        新任务
      </Button>
      <Button small icon={<Icon icon={IconNames.IMPORT} />} onClick={onBatch}>
        批量
      </Button>
    </div>
  )
