...
export default function SheetsDataTab({ batch, onBatchChange }: SheetsDataTabProps) {
  ...
  const filteredData = useMemo(() => {
    if (!batch || !batchKey) return data
    return data.filter(r => String(r[batchKey] ?? "") === batch)
  }, [data, batch, batchKey])
  ...
  useEffect(() => {
    if (data.length === 0 || !batchKey) return
    if (batch && !batchOptions.includes(batch)) onBatchChange("")
  }, [data, batchKey, batchOptions, batch, onBatchChange])
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", width: "100%" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "4px 8px", borderBottom: "1px solid var(--border)", flexShrink: 0 }}>
        ...
        <HTMLSelect value={batch} onChange={e => onBatchChange(e.target.value)} style={{ width: 120 }}>
          <option value="">批次</option>
          {batchOptions.map(v => <option key={v} value={v}>{v}</option>)}
        </HTMLSelect>
        ...
        <a href={`https://docs.google.com/spreadsheets/d/${SPREADSHEET_ID}/edit`} target="_blank" rel="noopener noreferrer" style={{ fontSize: 12, color: "var(--text-link)", textDecoration: "none" }}>
          <Icon icon={IconNames.SHARE} /> 在 Google Sheets 中打开
        </a>
      </div>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
            <thead>
              <tr style={{ borderBottom: "2px solid var(--border)", background: "var(--bg-hover)", position: "sticky", top: 0, zIndex: 1 }}>
                ...
                {headers.map(h => (
                  <th key={h} style={{ padding: "6px 8px", textAlign: "left", fontWeight: 600, fontSize: 11, textTransform: "uppercase", letterSpacing: "0.5px" }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {filteredData.map((row, i) => (
                <tr key={String(row.id ?? i)} style={{ borderBottom: "1px solid var(--border)" }}>
                  ...
                  {headers.map(h => (
                    <td key={h} style={{ padding: "2px 8px", whiteSpace: "nowrap" }}>{String(row[h] ?? "")}</td>
                  ))}
                </tr>
              ))}
            </tbody>
