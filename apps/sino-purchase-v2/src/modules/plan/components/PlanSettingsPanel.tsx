...
import { Button, InputGroup, NumericInput, Icon, Box, Stack, Text, H4, IconNames } from "../../../components/ui"
...
/**
 * 计划设置（右侧面板）：参考 v1 SettingsModal / v2 SettingsDialog。
...
export function PlanSettingsPanel() {
  ...
  const [rates, setRates] = useState<StoredRates>(() => {
    try { const s = localStorage.getItem(STORAGE_KEY); if (s) return JSON.parse(s) } catch { /* empty */ }
    ...
    fetch("https://open.er-api.com/v6/latest/ZMW", { signal: ctrl.signal }).then(r => r.json()).then(data => {
      if (data.rates?.USD != null && data.rates?.CNY != null) {
        const newRates: StoredRates = {
          ...
          timestamp: Date.now(), manualOverride: false,
        }
  ...
  function handleSaveRates() {
    ...
    if (usd > 0 && cny > 0) {
      ...
      setRates(newRates); persistRates(newRates)
    }
  }

  function handleRefresh() {
    setLoading(true)
    fetch("https://open.er-api.com/v6/latest/ZMW").then(r => r.json()).then(data => {
      if (data.rates?.USD != null && data.rates?.CNY != null) {
        const newRates: StoredRates = {
          rates: { ZMW: 1, USD: Math.round((1 / data.rates.USD) * 100) / 100, CNY: Math.round((1 / data.rates.CNY) * 100) / 100 },
          timestamp: Date.now(), manualOverride: false,
  ...
  return (
    <Stack spacing={1} sx={{ height: "100%", overflow: "auto", p: 1.5 }}>
      <BoxTitle>计划设置</BoxTitle>

      <H4 style={{ color: "var(--text-dim)" }}>默认汇率</H4>
      ...
      <H4 style={{ color: "var(--text-dim)", marginTop: 8 }}>起止日期 {dateStart && dateEnd ? `(${Math.round((new Date(dateEnd).getTime() - new Date(dateStart).getTime()) / 86400000) + 1})` : ""}</H4>
      <Stack direction="row" spacing={0.5} alignItems="center" sx={{ flexWrap: "wrap" }}>
        {[-5, -4, -3, -2, -1, 0].map(offset => {
          const d = new Date(); d.setMonth(d.getMonth() + offset)
          return (
            <Button key={offset} small minimal onClick={() => { const r = monthRange(offset); setDateStart(r.start); if (!dateEndToday) setDateEnd(r.end) }}>{d.getMonth() + 1}月</Button>
          )
        })}
      </Stack>
      <Stack direction="row" spacing={0.5} alignItems="center">
...
function BoxTitle({ children }: { children: ReactNode }) {
  return <Box sx={{ fontSize: 13, fontWeight: 600 }}>{children}</Box>
}
